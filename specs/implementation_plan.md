...
| D8 | Seeding halls from a school | When a school is approved, `halls_count` rows are created with `capacity = hall_capacity` and `safe_capacity = FLOOR(capacity × safe_utilization_rate)`. Real per-hall data may be imported later and overrides the seed. | `scenarios/service.py` |
